# 1. End the game when all balls are removed (Success)
# 2. End the game if the character collides with a ball (Failure)
# 3. End the game if the time limit (99 seconds) is exceeded (Failure)
...
# Time Over (failure due to timeout)
# Mission Complete (success)
# Game Over (failure due to character collision)
...
while running:
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        for weapon_idx, weapon_val in enumerate(weapons):
            ...
            if weapon_rect.colliderect(ball_rect):
                ...
                # Split the ball if it is not the smallest size
                if ball_img_idx < 3:
                    # Current ball size
    ...
    elapsed_time = (pygame.time.get_ticks() - start_ticks) / 1000  # ms -> seconds
    timer = game_font.render(
        "Time : {}".format(int(total_time - elapsed_time)),
        True,
        (255, 255, 255)
    )
...
msg = game_font.render(game_result, True, (255, 255, 0))  # yellow text
